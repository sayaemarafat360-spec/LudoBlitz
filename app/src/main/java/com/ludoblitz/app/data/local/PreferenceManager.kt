...
import com.ludoblitz.app.domain.gamification.UserProgress
...
class PreferenceManager @Inject constructor(
    ...
    // User Progress - Combined flow for gamification
    val userProgress: Flow<UserProgress> = context.dataStore.data
        .map { preferences ->
            UserProgress(
                coins = preferences[PreferencesKeys.COINS] ?: 500L,
                gems = (preferences[PreferencesKeys.GEMS] ?: 5L).toInt(),
                xp = (preferences[PreferencesKeys.XP] ?: 0L).toInt(),
                level = preferences[PreferencesKeys.LEVEL] ?: 1,
                gamesPlayed = preferences[PreferencesKeys.TOTAL_GAMES_PLAYED] ?: 0,
                gamesWon = preferences[PreferencesKeys.TOTAL_WINS] ?: 0,
                dailyRewardStreak = preferences[PreferencesKeys.DAILY_REWARD_DAY] ?: 0,
                lastDailyRewardClaim = preferences[PreferencesKeys.LAST_DAILY_REWARD] ?: 0L,
                equippedBoard = preferences[PreferencesKeys.SELECTED_BOARD] ?: "classic",
                equippedTokens = preferences[PreferencesKeys.SELECTED_TOKEN] ?: "default",
                equippedDice = preferences[PreferencesKeys.SELECTED_DICE] ?: "default"
            )
        }
    
    /**
     * Update user progress with a transformation function
     */
    suspend fun updateUserProgress(update: (UserProgress) -> UserProgress) {
        context.dataStore.edit { preferences ->
            val current = UserProgress(
                coins = preferences[PreferencesKeys.COINS] ?: 500L,
                gems = (preferences[PreferencesKeys.GEMS] ?: 5L).toInt(),
                xp = (preferences[PreferencesKeys.XP] ?: 0L).toInt(),
                level = preferences[PreferencesKeys.LEVEL] ?: 1,
            ...
            preferences[PreferencesKeys.COINS] = updated.coins
            preferences[PreferencesKeys.GEMS] = updated.gems.toLong()
            preferences[PreferencesKeys.XP] = updated.xp.toLong()
            preferences[PreferencesKeys.LEVEL] = updated.level
            preferences[PreferencesKeys.TOTAL_GAMES_PLAYED] = updated.gamesPlayed
            preferences[PreferencesKeys.TOTAL_WINS] = updated.gamesWon
            preferences[PreferencesKeys.DAILY_REWARD_DAY] = updated.dailyRewardStreak
            preferences[PreferencesKeys.LAST_DAILY_REWARD] = updated.lastDailyRewardClaim
            preferences[PreferencesKeys.SELECTED_BOARD] = updated.equippedBoard
            preferences[PreferencesKeys.SELECTED_TOKEN] = updated.equippedTokens
            preferences[PreferencesKeys.SELECTED_DICE] = updated.equippedDice
        }
    }
}
